...
#Zeiteinstellung: Wie viele Sekunden sollen die Lampen Leuchten?
onTime = 8
...
def handle_activate_led(data):
    ...
    if button_name == "buttonKobalt":
        active_led_from_Website('Kobalt')
    elif button_name == "buttonTantal":
        active_led_from_Website('Tantal')
    elif button_name == "buttonWolfram":
        active_led_from_Website('Wolfram')
    elif button_name == "buttonZinn":
        active_led_from_Website('Zinn')
    else:
        print("Ungültiger Button!")

# LED-Logik Website
def active_led_from_Website(button_name):
    try:
...
def active_led(button_name):
    try:
        ...
        for led_name in leds_to_activate.get(button_name, []):
            leds[led_name].on()
            logging.info(f"Alle LEDs für {button_name} eingeschaltet")
            
            
        def turn_off_leds():
            logging.info(f"LEDs für {button_name} ausschalten nach {onTime} Sekunden")
            for led_name in leds_to_activate.get(button_name, []):
                leds[led_name].off()
                
        led_timer = threading.Timer(onTime, turn_off_leds)
        logging.info(f"led_timer für {button_name} erstellt")
        led_timer.start()
        logging.info(f"led_timer für {button_name} gestartet")
